...
async function verifyCommunityStatus(user, force = false) {
  ...
  try {
    const response = await axios.post(user.community_status_url, {}, {
      timeout: 5000,
      maxRedirects: 0,
      headers: { Authorization: `Bearer ${user.status_token}`, 'User-Agent': 'CodeDog-IM-Status/1.0' },
      validateStatus: () => true
    });
    if (response.status !== 200 || response.data?.code !== 200 || !response.data?.data?.active) throw new Error(response.data?.msg || '账号状态不可用');
    cache.set(key, { expiresAt: Date.now() + CACHE_MS, data: response.data.data });
    return response.data.data;
  } catch (error) {
    cache.delete(key);
    throw Object.assign(new Error(error.response?.data?.msg || error.message || '无法确认编程狗账号状态'), { statusCode: 401 });
  }
}
