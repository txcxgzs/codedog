  ...
  <div class="admin-shell">
    ...
    <main>
      ...
      <template v-else-if="section === 'logs'">
        <section class="management-toolbar"><div><b>管理员操作日志</b><span>聊天检索、举报处置和群容量调整均不可删除</span></div><label class="select-label">操作类型<select v-model="auditAction" @change="loadAudits"><option value="">全部</option><option value="messages.search">聊天检索</option><option value="report.confirm">确认违规</option><option value="report.delete_message">删除消息</option><option value="report.delete_and_disable">删除并禁用</option><option value="group.member_limit.update">群容量调整</option></select></label></section>
...
const auditActionLabel = action => ({ 'messages.search':'聊天检索', 'report.reject':'驳回举报', 'report.confirm':'确认违规', 'report.delete_message':'删除违规消息', 'report.delete_and_disable':'删除消息并禁用账号', 'group.member_limit.update':'调整群容量' }[action] || action)
...
@media(max-width:1050px){.dashboard-grid{grid-template-columns:1fr 1fr}.dashboard-columns{grid-template-columns:1fr}.group-limit{grid-template-columns:1fr 1fr}.group-limit div{grid-column:1/-1}.report-meta{grid-template-columns:1fr 1fr}}@media(max-width:850px){.dashboard-grid{grid-template-columns:1fr}.management-toolbar{align-items:stretch;flex-direction:column}.management-toolbar label,.management-toolbar input{width:100%}.user-detail-grid{grid-template-columns:1fr 1fr}.summary-grid{grid-template-columns:1fr}.report-summary{grid-template-columns:1fr 1fr}.decision{grid-template-columns:1fr}.message-list article{grid-template-columns:1fr}.report-dialog>footer{flex-wrap:wrap}}
</style>
