...
async function getStudios(req, res) {
    try {
        ...
        await Promise.all(rows.filter(studio => {
            ...
            return !cover || (wasGeneratedByUs && !cover.endsWith('#codedog-studio-v5'));
        }).map(async (studio) => {
            try {
                const url = await generateAndUploadStudioCover(studio);
                await DbAdapter.update(Studio, { cover: url, cover_url: url }, { where: { id: studio.id } });
...
async function getStudioDetail(req, res) {
    try {
        ...
        const detailNeedsCover = !detailCover || (detailCover === studio.cover_url && !detailCover.endsWith('#codedog-studio-v5'));
        if (detailNeedsCover) {
            try {
                const url = await generateAndUploadStudioCover(studio);
                await DbAdapter.update(Studio, { cover: url, cover_url: url }, { where: { id: studio.id } });
