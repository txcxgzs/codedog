...
4. 生产环境使用 MySQL 保存持久消息，Redis 保存 SSO 防重放、在线状态和限流状态；当前生产仅部署一个 IM 服务节点。
...
9. 当前不实现跨节点消息总线；未来确认需要多节点时再增加 Redis Streams 或 NATS JetStream。
...
- 单条文本消息最大 10,000 字符。
- 支持文本和图片消息，不支持普通文件消息。
- 图片复用编程狗现有图床链路，格式和大小与现有 `/api/uploads/image` 保持一致：JPG、PNG、WebP、GIF，最大 5 MB。
...
推荐首发采用同域名路径，避免额外跨域复杂度：

```text
https://54188.xyz/              -> codedog-server（现有社区和静态前端）
https://54188.xyz/im/           -> im-web（独立构建的 IM 前端）
https://54188.xyz/im/admin/     -> im-admin（独立 IM 后台）
https://54188.xyz/im/api/       -> im-server HTTP API
wss://54188.xyz/im/ws           -> im-server WebSocket Gateway
```

根 `docker-compose.yml` 最终增加 `im-server`、`im-web`、`im-admin`、`mysql`、`redis` 服务。社区和 IM 可以共享 MySQL 实例，但必须使用不同数据库账号和不同逻辑数据库；IM 不能直接拥有社区表的写权限。

启动顺序：MySQL/Redis 健康 -> IM migrations -> IM server 健康 -> IM web/admin -> 编程狗入口开放。任一 IM 服务不可用时，编程狗社区仍能独立运行，入口显示“通讯系统维护中”，不能拖垮主站。
...
# 编程狗
IM_ENABLED=true
IM_PUBLIC_URL=https://54188.xyz/im
IM_INTERNAL_URL=http://im-server:3100
IM_SSO_PRIVATE_KEY_FILE=/run/secrets/im_sso_private_key

# IM
IM_PUBLIC_ORIGIN=https://54188.xyz
IM_PORT=3100
CODEDOG_INTERNAL_URL=http://codedog:3001
IM_SSO_PUBLIC_KEY_FILE=/run/secrets/im_sso_public_key
IM_DATABASE_URL=mysql://im_user:***@mysql:3306/codedog_im
IM_REDIS_URL=redis://redis:6379/1
```

SSO 使用非对称签名：编程狗只持有私钥，IM 只持有公钥。这样 IM 数据库或服务泄漏时，攻击者也不能伪造编程狗登录身份。

### 4.4 单独部署时的实际拓扑

```text
社区服务器：https://54188.xyz
IM 用户端： https://im.54188.xyz
IM 后台：   https://im-admin.54188.xyz
IM API：    https://im-api.54188.xyz
IM WS：     wss://im-api.54188.xyz/ws
```

独立部署只需把编程狗的 `IM_PUBLIC_URL` 改为 `https://im.54188.xyz`，并让两台服务器通过 HTTPS 内部接口通信。两边不需要共享文件目录、Cookie 或数据库。内部接口必须使用双向服务凭证、请求时间戳、nonce 和签名，拒绝重放。

### 4.5 两套系统如何连接

两套系统只有四条明确连接，不形成隐式数据库耦合：

1. **浏览器跳转**：编程狗导航入口跳到 `IM_PUBLIC_URL`。
2. **一次性 SSO**：编程狗签发 Ticket，IM 兑换为自己的登录会话。
3. **账号资料读取**：IM 使用内部服务凭证按用户 ID 批量读取昵称、头像、状态和角色。
4. **账号事件推送**：编程狗向 IM 推送资料变化、角色变化、禁用、密码重置和全端登出事件；IM 定时对账兜底。

IM 不调用编程狗的普通浏览器 API，不复制密码，不共用 Cookie，也不直接修改社区用户表。
...
IM 不保存文件或图片二进制，主要持久化的是文字消息和少量关系/可靠性数据。即使内容以文字为主，会话成员关系、消息顺序、群权限、撤回/编辑状态和审计记录仍必须落库，否则无法保证断线补拉和后台追溯。图片消息只保存现有图床 URL 与必要元数据。产品明确不提供已读回执。
...
- `im_images`：图床 URL、宽高、MIME、大小、hash、审核状态；不保存图片二进制。
- `im_blocks`：用户拉黑关系。
...
现有编程狗图片接口为 `POST /api/uploads/image`，底层由 `server/services/imageHost.js` 转发到 `https://img.scdn.io/api/v1.php`，使用 `img.scdn.io` CDN 和 `telegram` 存储目标。IM 沿用同一图床、字段和限制，不再建设对象存储：

1. IM 前端向 `POST /im/api/images` 上传图片。
2. IM 服务只接受 JPG、PNG、WebP、GIF，最大 5 MB，并增加文件魔数和实际图片解码校验。
3. IM 服务复用现有 `imageHost` 适配逻辑，将临时图片转发到同一图床。
4. 图床返回 URL 后立即删除 IM 本地临时文件。
5. IM 数据库只保存图床 URL、宽高、MIME、大小、hash 和审核状态。
6. 发送图片消息时只能引用当前用户刚上传且未被使用/过期的图片记录，不能任意提交外部 URL。
7. 图床失败时上传接口返回失败，不创建空白图片消息。

生产前应把现有图床地址、CDN 域名和存储目标从硬编码提取为两套系统共用的环境配置，但默认值保持现有行为。图片经过服务端短暂中转是为了沿用现有图床并完成安全校验，不做长期本地保存。
...
首期事件：`auth`、`auth.ok`、`ping`、`pong`、`message.send`、`message.ack`、`message.new`、`message.error`、`typing.start`、`typing.stop`、`presence.changed`、`conversation.updated`、`member.updated`、`session.revoked`。

## 8. 高并发策略

- Gateway 不保存业务真相，连接状态进入 Redis，并设置心跳 TTL。
- 一个用户允许多个设备连接，每个连接使用独立 connection id。
- 当前为单 IM 节点，实时消息使用进程内连接索引；Redis 不承担跨节点分发。
- 热点群采用批次 fan-out；不为每个离线成员预写一条消息副本。
- 历史消息使用 sequence 游标分页，禁止 `OFFSET 100000` 类查询。
- IM 不提供普通文件上传；图片经 IM 服务短暂中转到现有图床，成功后立即删除临时文件。
...
独立后台必须能清楚查看聊天记录，包括私聊和群聊：

- 可按用户 ID/昵称、会话 ID、群名称、消息 ID、关键词和时间范围检索。
- 结果展示发送者头像与账号、发送时间、消息正文、图片缩略图/原图链接、回复关系、编辑/撤回/删除状态。
- 可打开完整会话上下文，并向前向后加载消息，避免只有命中的孤立一句话。
- 被用户撤回的消息在普通客户端显示撤回提示；后台保留原内容并明确标记“已撤回”，用于举报和安全审计。
- 编辑消息保留版本记录，后台可以查看修改前后内容和时间。
- 管理员可以从记录直接执行警告、禁言、强制下线、封禁、隐藏违规消息或处置群聊。
- 图片只展示图床 URL，不把图片复制进 IM 数据库；图床图片失效时仍保留 URL、hash 和原始元数据。

查看聊天记录属于高敏感权限：

- 权限拆为 `messages:search`、`messages:view_content`、`messages:moderate`，默认不授予普通后台人员。
- 每次查看正文都记录管理员、查询条件、查看会话、原因、时间和来源 IP。
- 首次打开正文前必须填写查看原因；导出默认关闭，只有 `superadmin` 可临时授权。
- 聊天记录审计日志不可由具有聊天查看权限的同一管理员删除。
- 数据保留期限由后台配置；到期执行可验证的批量清理，举报冻结的消息例外保留到案件结束。
...
- 消息气泡保留清晰对比度，科幻装饰不影响长文本、代码和图片可读性。
...
- 私聊申请、文本消息、ACK、补拉、重连和同账号多连接同步，不实现已读回执。
...
- 图片、回复、编辑、撤回、回应和历史搜索。
- 验收：图片格式、魔数、大小和 hash 校验有效；图床失败不产生消息；IM 节点不残留临时图片。
...
为避免实现阶段继续悬而未决，M0 按以下基线开始；后续均可通过配置调整：

1. 陌生人可以发送一次会话申请，对方同意前不能继续发消息；用户可关闭陌生人申请。
2. 所有群默认统一限制 100 人，只有编程狗 `admin`、`superadmin` 可通过独立后台设置有审计记录的例外；`moderator` 默认无权执行。
3. 不开发语音消息、语音通话和视频通话。
4. 图片复用编程狗现有图床，不增加本机或 S3 长期图片存储；普通文件消息不支持。
5. 本地开发允许 SQLite/内存替代以降低门槛；生产 IM 必须新增 MySQL 和 Redis，不能依赖社区当前的 SQLite。
6. 编程狗主站只增加跳转和 SSO Ticket 接口；聊天用户端、服务端、迁移、压测和后台均放入 `im-system/`。
