    ...
    <div class="r-dev--container">
      ...
      <el-dialog v-model="dialogVisible" :title="editingId ? '编辑应用' : '创建应用'" width="min(720px, 94vw)" destroy-on-close :before-close="handleDialogBeforeClose">
        ...
        <el-form :model="form" class="r-dev--wizard_form" label-position="top">
          ...
          <el-form-item v-show="wizardStep === 1" label="回调 URL">
            <el-input
            ...
            />
            <div class="r-dev--upload_hint">仅申请应用级公开权限时可留空；申请用户级权限时必须填写。</div>
          </el-form-item>
...
const expandedScopeGroups = ref(['application'])
...
const scopeGroups = computed(() => [
  { key: 'application', title: '应用级公开数据', description: '无需用户授权，审核通过后由应用凭证访问', items: scopeOptions.value.filter(s => s.audience === 'application') },
  { key: 'identity', title: '身份与基础资料', description: '需要用户授权的身份、资料和通知', items: scopeOptions.value.filter(s => s.audience !== 'application' && s.risk === 'read' && /^(openid|profile|notifications)/.test(s.key)) },
  { key: 'analytics', title: '用户数据分析', description: '需要用户授权的作品、帖子和账号统计', items: scopeOptions.value.filter(s => s.audience !== 'application' && s.risk === 'read' && /analytics/.test(s.key)) },
  { key: 'studio', title: '工作室授权数据', description: '需要用户授权并校验工作室实际角色', items: scopeOptions.value.filter(s => s.audience !== 'application' && s.risk === 'read' && /^studios:/.test(s.key)) },
  { key: 'content', title: '个人内容读取', description: '需要授权，读取用户自己的作品、评论、帖子、收藏和动态', items: scopeOptions.value.filter(s => s.audience !== 'application' && s.risk === 'read' && !/^(openid|profile|notifications|studios:)/.test(s.key) && !/analytics/.test(s.key)) },
  { key: 'write', title: '写入操作', description: '会修改用户数据，请谨慎申请', items: scopeOptions.value.filter(s => s.audience !== 'application' && s.risk === 'write') },
  { key: 'admin', title: '管理权限', description: '高风险审核和管理能力', items: scopeOptions.value.filter(s => s.audience !== 'application' && s.risk === 'admin') }
...
const nextWizardStep = () => {
  ...
  if (wizardStep.value === 1 && form.redirect_uris_text.trim()) { const result = validateRedirectUris(); if (!result.ok) return ElMessage.warning(result.message) }
...
const authExample = (app) => {
  ...
  const scope = (app.scopes_requested || ['profile:read']).filter(key => !applicationScopeKeys.value.has(key)).join(' ')
  if (!scope) return '该应用仅申请应用级公开权限，无需生成用户授权链接。请使用 client_credentials 获取应用令牌。'
...
const submitForm = async () => {
  ...
  let redirect_uris = []
  if (requiresUserAuthorization.value || form.redirect_uris_text.trim()) {
    ...
    redirect_uris = redirectResult.uris
  }
  let geetestData = {}
